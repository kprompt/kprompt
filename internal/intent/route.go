...
var (
	...
	routableClause    = regexp.MustCompile(
		`(?i)\b(?:deploy|install|upgrade|scale|rollback|undo|list|show|get|explain|logs?|describe|train|workflow|slow|latency|performance|trace|dashboards?|optimize|right\s*siz|delete|remove)\b`,
	)
)
